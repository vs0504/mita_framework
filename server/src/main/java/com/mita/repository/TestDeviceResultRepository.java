...
public interface TestDeviceResultRepository extends JpaRepository<TestDeviceResult, Long> {
  ...
  @Query("SELECT DISTINCT(envResult.id) " +
    ...
    "AND tsr.status = com.mita.model.StatusConstant.STATUS_QUEUED " +
  ...
  @Query("SELECT " +
    "MAX(" +
    " CASE " +
    "   WHEN envrResult.result = com.mita.model.ResultConstant.SUCCESS THEN 0 " +
    "   WHEN envrResult.result = com.mita.model.ResultConstant.FAILURE THEN 1 " +
    "   WHEN envrResult.result = com.mita.model.ResultConstant.ABORTED THEN 2 " +
    "   WHEN envrResult.result = com.mita.model.ResultConstant.NOT_EXECUTED THEN 3 " +
    "   WHEN envrResult.result = com.mita.model.ResultConstant.QUEUED THEN 4 " +
    "   ELSE 5 " +
